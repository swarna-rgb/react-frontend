...
class App extends Component {
  ...
  async handleSubmit(e){
    ...
    e.preventDefault()
    try {
      const {data} = await axios.post('http://127.0.0.1:8000/toys/',{name: newToyName, category: newToyCategory})
      ...
      <div className="App">
        <div>
          <form onSubmit={this.handleSubmit}>
            <input onChange={this.nameChange} name={newToyName} value={newToyName}/>
            <input onChange={this.categoryChange} name={newToyCategory} value={newToyCategory}/>
        ...
        <ul>
          {
            ...
            <li key={id}>
              ...
              <button onClick={(event) => this.deleteToy(id)}>Delete Toy</button>              
